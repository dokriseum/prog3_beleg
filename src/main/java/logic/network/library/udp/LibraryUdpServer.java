package logic.network.library.udp;

import logic.network.library.Library;
...
public class LibraryUdpServer {
    ...
    private void deliver(Session session, int messageId, InetAddress address, int port) {
        if (!library.lineExists(session.bookId, session.line))
            this.sendResponse('E', messageId, session.id, null, address, port);
        else
            this.sendResponse('L', messageId, session.id, this.library.getLine(session.bookId, session.line++), address, port);
    }
